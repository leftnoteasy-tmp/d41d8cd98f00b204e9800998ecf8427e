...
int get_new_app_impl(hadoop_rpc_proxy_t* proxy) {
    int rc;
    if (-1 != proxy->app_id) {
        opal_output(0, "there's already a app, id=%d, will exit\n", proxy->app_id);
        return -1;
    }
    ...
    if (0 != rc) {
        opal_output(0, "generate new app request failed.\n");
        return -1;
    }
    ...
    if (0 != rc) {
        opal_output(0, "send new_application_request failed.\n");
        free(request);
        return -1;
    }
    ...
    if (RESPONSE_SUCCEED == response_type) {
        ...
        if (!rmsg) {
            opal_output(0, "deserialize GetNewApplicationResponseProto from buffer failed.\n");
            free(slice.buffer);
            return -1;
        }
        struct pbc_rmessage* id = pbc_rmessage_message(rmsg, "application_id", 0);
        if (!id) {
            opal_output(0, "deserialize application_id from GetNewApplicationResponseProto failed.\n");
